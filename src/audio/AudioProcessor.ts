export class AudioProcessor {
  ...
  private async decodeWithFFmpeg(arrayBuffer: ArrayBuffer, filename: string): Promise<AudioBuffer> {
  ...
  private async getFFmpegCoreURL(): Promise<string> {
    ...
    const cdnUrl = 'https://unpkg.com/@ffmpeg/core@0.12.6/dist/umd/ffmpeg-core.js';
  ...
  private async getFFmpegWasmURL(): Promise<string> {
    // Try to load from CDN first, fallback to local
    const cdnUrl = 'https://unpkg.com/@ffmpeg/core@0.12.6/dist/umd/ffmpeg-core.wasm';
  ...
  createAudioBuffer(channelData: Float32Array[], sampleRate: number): AudioBuffer {
